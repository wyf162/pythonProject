...
for _tcn_ in range(tcn):
    n = I()
    nums = LI()
    distinct_nums = list(sorted(nums))
    v2i = {v: 2 * i + 1 for i, v in enumerate(distinct_nums)}
    # print(v2i)
    Lst = LazySegmentTree(len(distinct_nums) * 2 + 6, max, 0, max, max, 0)
    f = []
    for x in nums:
        i = v2i[x]
        mx = Lst.prod(0, i)
