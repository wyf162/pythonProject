...
for _tcn_ in range(tcn):
    n = I()
    A = LI()
    Lst = LazySegmentTree(n + 5, max, 0, max, max, 0)
    left_larger = [-1] * n
    stack = [-1]
    for i in range(n):
    ...
    for i in sorted(range(n), key=lambda x:A[x]):
        if left_larger[i] != -1:
            dp[i+1] = max(Lst.prod(0, left_larger[i] + 1) + 2, Lst.prod(left_larger[i] + 1, i + 1) + 1)
        else:
            dp[i+1] = Lst.prod(0, i + 1) + 1
        Lst.set(i + 1, dp[i+1])
    print(max(dp))
