...
for _tcn_ in range(tcn):
    ...
    while L <= R:
        mid = (L + R) // 2
        fmin = lambda x1, x2: x1 if x1 < x2 else x2
        Lst = LazySegmentTree(n + 1, fmin, inf, fmin, fmin, inf)
        Lst.set(0, 0)

        for i in range(n):
            i1 = bisect.bisect_left(PA, max(PA[i] - mid, 0))
            mi = Lst.prod(i1, i + 1)
            Lst.set(i + 1, mi + A[i])
        ...
        cur = 0
        ans = False if B[n-1] > mid else True
